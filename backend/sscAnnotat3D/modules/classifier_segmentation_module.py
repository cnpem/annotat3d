...
class ClassifierSegmentationModule(SegmentationModule):
    def __init__(
        self, image, auto_save=False, workspace=os.path.join(Path.home().absolute().as_posix()), parent=None, **kwargs
    ):
    ...
    def _create_grid_search(grid_params, base_model):
        return model_selection.GridSearchCV(
            cv=3, estimator=base_model, param_grid=grid_params, n_jobs=-1, scoring=gridsearch_scorer
        )
    ...
    def _train_classifier(self, annotations, superpixel_features, **kwargs):
        ...
        if len(annotations) > 0 or len(self._loaded_training_superpixel_features) > 0:
            ...
            sentry_sdk.set_context(
                "Feature params",
                {"dtype": self._training_features_raw.dtype, "shape": self._training_features_raw.shape},
            )
            ...
            with sentry_sdk.start_span(op="normalize data"):
                with parallel_backend("threading"):
                    ...
                    spin_feat_extraction.spin_standardize_feature_vectors(
    ...
    def save_training_data(self, path):
        if not (
            (len(self._loaded_training_superpixel_labels) != 0 and len(self._loaded_training_superpixel_labels) != 0)
            or (len(self._training_labels_raw) != 0 and len(self._training_features_raw) != 0)
        ):
    ...
    def load_training_data(self, path):
        ...
        try:
            ...
            raise Exception(
                "Invalid training data file! Unable to load feature extraction parameters! (Error: %s)" % str(e)
            )
        try:
            loaded_training_superpixel_labels = training_data["superpixel_labels"]
        except Exception as e:
            raise Exception(
                "Invalid training data file! Unable to load training superpixel labels! (Error: %s)" % str(e)
            )
        ...
        try:
            ...
            raise Exception(
                "Invalid training data file! Unable to load training superpixel features! (Error: %s)" % str(e)
            )

        nsamples = len(loaded_training_superpixel_features)
        nlabels = len(loaded_training_superpixel_labels)
        nfeats = (
            loaded_training_superpixel_features.shape[1]
            if type(loaded_training_superpixel_features) == np.ndarray
            else len(loaded_training_superpixel_features[0])
        )
    ...
    def reset_classifier(self):
        ...
        feat_selection_method_params = self._extract_parameters_from_dict(
            self._feature_extraction_params, "feat_selection_method"
        )
        feat_selection_clf_params = self._extract_parameters_from_dict(
            self._feature_extraction_params, "feat_selection_clf"
        )
        self._default_feat_selector = SelectFromModel(
            ExtraTreesClassifier(**feat_selection_clf_params), **feat_selection_method_params
        )
        ...
        for c in self._available_classifiers:
            ...
            self._available_classifiers[c] = (
                clf  # Pipeline([('feature_selection', feat_selection), ('classification', clf)])
            )
    ...
    def _scale_feats(self, feats):
        if self._feat_scaler is not None:
            ...
            spin_feat_extraction.spin_standardize_feature_vectors(
                feats, X_scaled, self._feat_scaler.mean_, self._feat_scaler.scale_
            )
    ...
    def _select_feats(self, feats):
        if self._feat_selector is not None and self._feature_extraction_params["feat_selection_enabled"]:
            ...
            logging.debug(
                "---- Old prediction superpixel features shape: {} Selected features shape:".format(
                    old_shape, new_shape
                )
            )
        else:
    ...
    def _validate_feature_extraction_memory_usage(self, superpixel = True,**kwargs):
        ...
        if not has_enough_memory:
            logging.debug(
                "** WARNING: Estimated amount of memory (%lu bytes) exceeds the threshold of %lu bytes"
                % (estimated_memory, available_memory)
            )
        else:
            logging.debug(
                "** Estimated amount of memory (%lu bytes) within threshold of %lu bytes"
                % (estimated_memory, available_memory)
            )
