...
class PixelSegmentationModule(ClassifierSegmentationModule):
    ...
    def _extract_voxel_features_for_training_by_annotations(
            self, annotations, **kwargs):
        image = self._image.astype(
            'int32') if self._image.dtype != 'int32' else self._image
        ...
        self._training_features_raw = np.empty((n_annotations, n_feats),
                                               dtype='float32')
        ...
        for idx, (coord, (label, mk_id)) in enumerate(list(annotations.items())):
            self._training_features_raw[idx] = pixel_map[(slice(None),
                                                          *(coord - bbox_min))]  # get all features from coordinate
    ...
    def _extract_voxel_features_for_training(self, annotations, pixel_features, **kwargs):
        if pixel_features is None:
            self._extract_voxel_features_for_training_by_annotations(annotations, **{
                **kwargs,
