...
const SegmentationMenu: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (['superpixel', 'pixel'].includes(segModule)) {
            const isVisible = showSuperpixel;
            dispatch('superpixelVisibilityChanged', isVisible);
            setShowSuperpixel(isVisible);
        }
    }, [segModule, showSuperpixel]);
