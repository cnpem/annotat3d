...
const EditLabelMenu: React.FC = () => {
    ...
    const [isEditLabelDisabled, setIsEditLabelDisabled] = useStorageState<boolean>(
        sessionStorage,
        'isEditLabelDisabled',
        false
    );
     ...
     */
    const reseEditLabel = (turnMergeSplitFalse = false) => {
        if (isMergeActivated) {
            ...
            sfetch('POST', '/set_edit_label_options', JSON.stringify(payloadMerge), 'json').catch(
            ...
            sfetch('POST', '/set_edit_label_options', JSON.stringify(payloadSlipt), 'json').catch(
                (error: ErrorInterface) => {
                        ...
                        <div hidden={!toggleEditLabel}>
                            ...
                            <IonItem>
                                ...
                                <IonCheckbox
                                    ...
                                    slot={'end'}
                                    onIonChange={(e: CustomEvent) => {
                                        dispatch('isSplitActivated', e.detail.checked);
